...
public class LinearActuator extends TalonSRX {
  ...
  public void PIDupdate(double posInInches) {
    super.setPID(SmartDashboard.getDouble("Linear Actuator P"),
                 SmartDashboard.getDouble("Linear Actuator I"),
                 SmartDashboard.getDouble("Linear Actuator D"));

    super.PIDPosition(clampInches(posInInches) * COUNTS_PER_INCH +
                      62); // conversion
    // to
    // rotations

    SmartDashboard.sendData("Talon " + super.getDeviceID(),
                            "Position(inches)" + posInInches);
    SmartDashboard.sendData("Talon " + super.getDeviceID(),
                            "Position(rotations)" + super.getPosition());

    SmartDashboard.sendData("Linear Actuator error", super.getError());
  }

  public double clampInches(double posInInches) {
    if (posInInches > MAX_INCHES) posInInches = MAX_INCHES;
    if (posInInches < MIN_INCHES) posInInches = MIN_INCHES;
    return posInInches;
  }
