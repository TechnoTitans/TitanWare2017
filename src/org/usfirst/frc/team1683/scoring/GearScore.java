...
public class GearScore {
	...
	public void run() {
		...
		SmartDashboard.sendData("gear score encoder distance", mover.getAverageDistanceLeft());
		SmartDashboard.sendData("gear score distance anomoly", difference);
		double confidence = vision.getConfidence() * 1/(1 + difference*difference*DIFFERENCE_WEIGHT);
		SmartDashboard.sendData("gear score confidence", confidence);
		if (confidence > 0.6) {
			if (distanceTarget < 10) speed = 0.2;
			if (distanceTarget < 5) {
				done = true;
				return;
			}
			double error = vision.getOffset();
			SmartDashboard.sendData("gear score offset", error);
			driveTrain.set(speed*(1-error*ERROR_KP), speed*(1+error*ERROR_KP));
		} else {
			mover.runIteration();
