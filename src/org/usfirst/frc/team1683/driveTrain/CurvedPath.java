...
public class CurvedPath extends CurvedDrive {
	...
	public CurvedPath(TankDrive drive, Gyro gyro, PathPoint[] path) {
		...
		if (path[0].getDistance() != 0) throw new IllegalArgumentException("The first point in the path must be the origin");
	...
	private Cubic[] interpolateCubics(double[][] pts) {
		...
		for (i = 0; i < pts.length - 2; ++i) {
			...
			double[][] matr = new double[][] {
				{3*x*x, 2*x, 1, 0, slope},
				{x*x*x, x*x, x, 1, pts[i][1]},
				{x2*x2*x2, x2*x2, x2, 1, pts[i+1][1]},
				{x3*x3*x3, x3*x3, x3, 1, pts[i+2][1]}
			};
	...
	private double[] solveSystem(double[][] mat) {
		int n = mat.length;
	    if (mat[0].length != (n + 1)) throw new Error("Cannot solve system");
	    for (int k = 0; k < n; ++k) {
	       int maxCoeff = k;
	       for (int i = k + 1; i < n; ++i) {
	           if (Math.abs(mat[i][k]) > Math.abs(mat[maxCoeff][k])) maxCoeff = i;
	       }
	       double[] tmp = mat[k];
	       mat[k] = mat[maxCoeff];
	       mat[maxCoeff] = tmp;
	       if (mat[k][k] == 0) throw new Error("Cannot solve system; no or infinitely many solutions exist");
	       for (int i = k + 1; i < n; ++i) {
