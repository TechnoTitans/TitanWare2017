...
public class PiVisionReader {
	...
	private static class VisionValue {
		...
		void update() {
			...
			if (confidence > 0) {
				if (!receivedOne) {
					// If this is the first value, there is no other value to go off of
					value = inp;
					receivedOne = true;
				}
	...
	public PiVisionReader() {
		...
		targetCenter = new VisionValue("Target_X", table, 0.6);
	...
	/**
	 * 
	 * @return An offset between -0.5 and 0.5 where negative indicates target is on the left
	 */
