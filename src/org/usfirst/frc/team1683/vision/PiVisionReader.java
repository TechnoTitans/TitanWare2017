...
public class PiVisionReader {
	...
	private static class Camera {
		...
		VisionValue getGoodCenter() {
			if (cx1.getConfidence() > CONFIDENCE_THRESHOLD)
				return cx1;
			else if (cx2.getConfidence() > CONFIDENCE_THRESHOLD)
				return cx2;
			else
				return null;
		}
		 ...
		 * 
		 * @param useCx1
		 *            If true, will use only cx 1 (left center), otherwise will
		 *            use only cx 2
		 * @return
		 */
	...
	public double getOffset() {
		...
		if (right.isUnconfident() && left.isUnconfident())
			return -1;
